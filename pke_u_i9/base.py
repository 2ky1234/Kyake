...
from pke_u_i9.data_structures import Candidate, Document
from pke_u_i9.readers import MinimalCoreNLPReader, RawTextReader, Reader
...
from pke_u_i9.langcodes import LANGUAGE_CODE_BY_NAME
...
def is_corenlp(input):
    return is_file_path(input) and input.endswith('.xml')  # 유효한 path인지 확인 / xml인지 확인
...
class LoadFile(object):
    ...
    def load_document(self, input, **kwargs):  # 1번
        ...
        if is_corenlp(input):  # 유효한 path  +  xml 파일 이라면..... (입력형태가 xml file 형태)
            path = input
            parser = MinimalCoreNLPReader()  # xml 파일을 parser하는 함수
            doc = parser.read(path=input, **kwargs)  # xml파일을 parser하여 text로 읽어 들인다.
            doc.is_corenlp_file = True

        elif is_file_path(input):  # 유효한 path 라면 (입력형태가 text file 형태)
            path = input
            with open(path, encoding=kwargs.get('encoding', 'utf-8')) as f:  # text 파일을 text로 읽어 들인다.
                input = f.read()
            parser = RawTextReader(language=language)  # 분석할 language 입력
            doc = parser.read(text=input, path=path,
                              **kwargs)  # 읽어드린 text language에 따라 RawTextReader함수를 통해 tokenizer하는 부분

        elif isinstance(input, str):

            parser = RawTextReader(language=language)  # 분석할 language 입력
            doc = parser.read(text=input, **kwargs)  # 읽어드린 text language에 따라 RawTextReader함수를 통해 tokenizer하는 부분
        ...
        self.stoplist = get_stopwords(self.language)  # stopword 초기화 입력
        ...
        if self.language == 'ko':
            ...
            for sentence in self.sentences:
                for idx, pos in enumerate(sentence.pos):
                    ...
                    if pos not in ('NOUN', 'ADJ', 'VERB'):  # 'NOUN','ADJ','VERB' 가 아니면  stop word로 다 추가한다.
    ...
    def is_redundant(self, candidate, prev, minimum_length=1):  # levenshtein distance
    ...
    def get_n_best(self, n=10, redundancy_removal=False, stemming=False):  # 이거 아님
