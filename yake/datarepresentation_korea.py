...
class DataCore(object):
    ...
    def _build(self, text, windowsSize, n):
        ...
        #self.sentences_str = [(i.stems).remove() for i in doc[0].sentences]
        
        # Yake직접 작성한 변경함수
        #[s for s in list(split_multi(text)) if len(s.strip()) > 0]
        #test_value = [s.split(' ') for s in list(split_multi(text)) if len(s.strip()) > 0]
