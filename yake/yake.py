...
def isParagraphReDuplicated(dataset, COpy):    # input: list(tuple)
    ...
    for x in range(len(dataset)):
        if dataset[x] not in copySet:
            continue

        for y in range(len(dataset)):
            if x==y:
                continue
            elif dataset[y] not in copySet:
                continue
            else:
                score = jacc_for_paragraph(dataset[x][1].kw, dataset[y][1].kw)
                if score >= 0.5:
                    try:
    ...
    for x in range(len(dataset)):
        ...
        for y in range(len(dataset)):
            if x==y:
                ...
                score = jacc_for_word(dataset[x][1].kw, dataset[y][1].kw)
                if score >= 0.5:
                    try:
                        if dataset[x][1].H > dataset[y][1].H:
                            copySet.remove(dataset[x])
...
class KeywordExtractor(object):
    ...
    def extract_keywords(self, text):
        ...
        try:
            ...
            return [(cand.origin_terms,h) for (h,cand) in finalReturnSet]  # [(cand.kw, h) for (h,cand) in resultSet]
