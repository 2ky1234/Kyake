...
if platform.system() == "Windows":
    try:
        # from eunjeon import Mecab # from konlpy.tag import Komoran    # from eunjeon import Mecab
        pass
    except:
        print("please install eunjeon module")
else:  # Ubuntu일 경우
    from konlpy.tag import Mecab    # Mecab # Komoran
...
# 위 문장:
        ...
        #     self.metrics = metrics[:]               # self.rouge_evaluator = Rouge( @@ metrics=["rouge-n", "rouge-l"] @@ , ..., ...)
        # else:
        #     self.metrics = Rouge.DEFAULT_METRICS    # DEFAULT_METRICS : {"rouge-n"}
        ###

        for m in self.metrics:                      # self.metrics : ["rouge-n", "rouge-l"]
...
# 위 문장:
        ###
        # if "rouge-n" in self.metrics:
        #     self.max_n = max_n                      # self.rouge_evaluator = Rouge(metrics=["rouge-n", "rouge-l"], @@ max_n=2, @@ , ..., ...)
        # else:                                       # self.max_n : 2
        #     self.max_n = None
        ###
        ...
        if self.max_n is not None:
            index_rouge_n = self.metrics.index("rouge-n")   # index_rouge_n : 0
            del self.metrics[index_rouge_n]
            self.metrics += ["rouge-{}".format(n) for n in range(1, self.max_n + 1)]
            # 위 문장:
            ###
            for n in range(1, self.max_n + 1):              # for n in range(1, 3):
                self.metrics.append("rouge-{}".format(n))   # self.metrics : [rouge-l, rouge-1, rouge-2]
            ###

        self.metrics = set(self.metrics)    # self.metrics : {rouge-l, rouge-1, rouge-2}


        self.limit_length = limit_length    # self.rouge_evaluator = Rouge(metrics=["rouge-n", "rouge-l"], max_n=2, @@ limit_length=True @@ , ..., ...)
        if self.limit_length:               # self.limit_length : True
            if length_limit_type not in Rouge.AVAILABLE_LENGTH_LIMIT_TYPES:     # length_limit_type : "words"
                raise ValueError("Unknown length_limit_type '{}'".format(length_limit_type))


        self.length_limit = length_limit                # self.length_limit : 1000      ## @@ length_limit=1000 @@ , ..., ..
        if self.length_limit == 0:
            self.limit_length = False
        self.length_limit_type = length_limit_type      # self.length_limit_type : "words"      ## @@ length_limit_type="words" @@ ,


        self.use_tokenizer = use_tokenizer              # use_tokenizer=True,
        if use_tokenizer:
            self.tokenizer = Mecab()  # self.tokenizer = Mecab()


        self.apply_avg = apply_avg                      # apply_avg=True,
        self.apply_best = apply_best                    # apply_best=False,
        self.alpha = alpha                              # alpha=0.5,  # Default F1_score
        self.weight_factor = weight_factor              # weight_factor=1.2,
        if self.weight_factor <= 0:
            raise ValueError("ROUGE-W weight factor must greater than 0.")


    def tokenize_text(self, text):
        if self.use_tokenizer:                          # self.use_tokenizer : True
            return self.tokenizer.morphs(text)          # return self.mecab.morphs(text)
        else:
            return text
    ...
    def _get_scores_rouge_n(self, all_hypothesis, all_references):
        # print("all_hypothesis: ",all_hypothesis)
        ...
        if self.apply_avg or self.apply_best:
            ...
            scores = {metric: [{stat: [] for stat in Rouge.STATS} for _ in range(len(all_hypothesis[0]))] for metric in metrics}
        ####    위 문장    ####
        # scores = {}
        # if self.apply_avg or self.apply_best:
        ...
        #                 stat = []
        #                 list001.append(stat)
        #             scores[metric] = list001
                    # scores[metric] = [{stat: [] for stat in Rouge.STATS}]
        ####              ####



        for sample_id, (hypothesis, references) in enumerate(zip(all_hypothesis[0], all_references[0])):
            # if not isinstance(hypothesis, str):
                # print("타입: ",type(hypothesis))
                # print("원문: ",hypothesis)
        ...
        if (self.apply_avg or self.apply_best) and len(all_hypothesis[0]) > 1:
            for metric in metrics:
                for stat in Rouge.STATS:
                    scores[metric][stat] /= len(all_hypothesis[0])
    ...
    def _get_scores_rouge_l_or_w(self, all_hypothesis, all_references, use_w=False):
        ...
        if self.apply_avg or self.apply_best:
            ...
            scores = {
                metric: [{stat: [] for stat in Rouge.STATS} for _ in range(len(all_hypothesis[0]))]
            }

        # print("all_hypothesis: ", all_hypothesis)
        # print("all_references: ", all_references)
        for sample_id, (hypothesis_sentences, references_sentences) in enumerate(
            zip(all_hypothesis[0], all_references[0])
        ):
